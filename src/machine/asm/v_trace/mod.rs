...
fn handle_vluxei8_256<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let offset = E8::get_unsafe(m.inner.v_ref(i.vs2(), 8, j, 1)).u64();
        m.inner
...
fn handle_vmadc_256<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E256::get_unsafe(m.inner.v_ref(i.vs2(), sew, j, 1));
        let a = E256::get_unsafe(m.inner.v_ref(i.vs1(), sew, j, 1));
        if alu::madc(b, a) {
            m.set_bit(i.vd(), j);
        } else {
            m.clr_bit(i.vd(), j);
...
fn handle_vmadc_512<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E512::get_unsafe(m.inner.v_ref(i.vs2(), sew, j, 1));
        let a = E512::get_unsafe(m.inner.v_ref(i.vs1(), sew, j, 1));
        if alu::madc(b, a) {
            m.set_bit(i.vd(), j);
        } else {
            m.clr_bit(i.vd(), j);
