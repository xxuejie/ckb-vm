...
fn handle_vadd_256<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E256::get_unsafe(m.element_ref(i.vs2(), 256, j));
        let a = E256::get_unsafe(m.element_ref(i.vs1(), 256, j));
...
fn handle_vadd_512<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E512::get_unsafe(m.element_ref(i.vs2(), 512, j));
        let a = E512::get_unsafe(m.element_ref(i.vs1(), 512, j));
        let r = Eint::wrapping_add(b, a);
...
fn handle_vmadc_256<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E256::get_unsafe(m.element_ref(i.vs2(), sew, j));
        let a = E256::get_unsafe(m.element_ref(i.vs1(), sew, j));
        if alu::madc(b, a) {
            m.set_bit(i.vd(), j);
        } else {
            m.clr_bit(i.vd(), j);
...
fn handle_vmadc_512<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E512::get_unsafe(m.element_ref(i.vs2(), sew, j));
        let a = E512::get_unsafe(m.element_ref(i.vs1(), sew, j));
        if alu::madc(b, a) {
            m.set_bit(i.vd(), j);
        } else {
            m.clr_bit(i.vd(), j);
...
fn handle_vnsrl_256<'a>(m: &mut CM<'a>, inst: Instruction) -> Result<(), Error> {
    ...
    for j in 0..m.vl() as usize {
        ...
        let b = E512::get_unsafe(m.element_ref(i.vs2(), sew * 2, j));
        let a = if 0 != 0 {
