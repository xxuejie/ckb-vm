...
static const unsigned char bf_actions[1198] = {
  ...
  247,255,15,131,244,247,255,15,182,12,2,255,15,183,12,2,255,139,12,2,255,72,
...
AotContext* aot_new(uint32_t npc, uint32_t version)
{
  ...
  //|.code
  dasm_put(Dst, 0);
#line 256 "src/machine/aot/aot.x64.c"
  //| push r12
  //| push r13
  //| push r14
  //| push r15
...
#line 274 "src/machine/aot/aot.x64.c"
  return context;
}
...
#line 305 "src/machine/aot/aot.x64.c"
   /*
...
#line 316 "src/machine/aot/aot.x64.c"
...
#line 324 "src/machine/aot/aot.x64.c"
   /*
    * Test if the page stored in rcx is out of bound, and if the page has
    * correct write permissions
    */
...
#line 463 "src/machine/aot/aot.x64.c"
      break;
  }
...
int aot_memory_read(AotContext* context, uint32_t target, AotValue address, uint32_t size)
{
  ...
  dasm_put(Dst, 1102, size, CKB_VM_ASM_RISCV_MAX_MEMORY);
...
static int aot_mov_internal(AotContext* context, riscv_register_t target, AotValue value, x64_register_t x64_temp_reg)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1) && VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1148, (loc2), (loc1));
      } else if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1156, (loc1), Dt1(->registers[value.value.reg]));
      } else if (VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1043, (loc2), Dt1(->registers[target]));
      } else {
      dasm_put(Dst, 1164, (x64_temp_reg), Dt1(->registers[value.value.reg]), (x64_temp_reg), Dt1(->registers[target]));
      }
#line 1315 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
        loc1 = riscv_reg_to_x64_reg(target);
      dasm_put(Dst, 1179, (x64_temp_reg), value.value.i >> 32, (x64_temp_reg), (x64_temp_reg), value.value.i & 0xFFFFFFFF);
        if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1148, (x64_temp_reg), (loc1));
        } else {
...
#line 1318 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      loc1 = riscv_reg_to_x64_reg(target);
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1148, (value.value.x64_reg), (loc1));
      } else {
...
static int aot_mov_x64(AotContext* context, x64_register_t x64_target, AotValue value)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1148, (loc1), (x64_target));
      } else {
      dasm_put(Dst, 1156, (x64_target), Dt1(->registers[value.value.reg]));
      }
#line 1334 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
      dasm_put(Dst, 1179, (x64_target), value.value.i >> 32, (x64_target), (x64_target), value.value.i & 0xFFFFFFFF);
      } else {
...
#line 1337 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1148, (value.value.x64_reg), (x64_target));
